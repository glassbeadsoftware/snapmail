...
const electron_prompt_1 = tslib_1.__importDefault(require("electron-prompt"));
const auto_launch_1 = tslib_1.__importDefault(require("auto-launch"));
const electron_updater_1 = require("electron-updater");
const electron_is_dev_1 = tslib_1.__importDefault(require("electron-is-dev"));
/** Holochain Modules */
//import {AdminWebsocket} from "@holochain/client";
//import {CellId} from "@holochain/client";
const electron_holochain_1 = tslib_1.__importStar(require("@lightningrodlabs/electron-holochain"));
/** My Modules */
...
const holochain_1 = require("./holochain");
...
if (process.platform === "win32") {
    const BIN_PATH = path.join(constants_1.CURRENT_DIR, "bin");
    logger_1.log('debug', 'BIN_PATH = ' + BIN_PATH);
...
let g_shutdown = undefined; // FIXME
let g_startingHandle;
...
let g_userSettings;
let g_uidList = [];
let g_networkSettings = {
    canProxy: true,
    canMdns: false,
    bootstrapUrl: constants_1.DEFAULT_BOOTSTRAP_URL,
    proxyUrl: constants_1.DEFAULT_PROXY_URL,
};
/**********************************************************************************************************************/
/*  SETUP
/**********************************************************************************************************************/
...
electron_updater_1.autoUpdater.on('update-available', (info) => {
    electron_1.dialog.showMessageBox({
        ...
        if (buttonIndex.response === 0) {
            electron_updater_1.autoUpdater.downloadUpdate().then((paths) => { logger_1.log('debug', 'download array: ' + JSON.stringify(paths)); });
        }
        else {
...
function checkForUpdates(menuItem) {
    if (constants_1.IS_DEBUG || electron_is_dev_1.default) {
        electron_1.dialog.showMessageBox({
...
ipc.on('startingInfo', async (event, startingHandle, dnaHash) => {
    ...
    logger_1.log('info', "startingHandle = " + startingHandle);
    ...
    if (g_startingHandle === "<noname>") {
        ...
        logger_1.log('info', "firstUsername set: " + firstUsername);
    }
...
ipc.on('newCountAsync', (event, newCount) => {
    ...
    if (g_tray) {
        g_tray.setToolTip('SnapMail v' + electron_1.app.getVersion() + append);
    }
    event.returnValue = true;
});
ipc.on('exitNetworkStatus', (event) => {
    const indexUrl = constants_1.INDEX_URL + g_appPort + '&UID=' + g_uid;
    g_mainWindow === null || g_mainWindow === void 0 ? void 0 : g_mainWindow.loadURL(indexUrl);
});
/**
 * Receive and reply to asynchronous message
 */
ipc.on('bootstrapStatus', (event) => {
    const succeeded = spawn_1.pingBootstrap(g_networkSettings.bootstrapUrl);
...
function createTray() {
    let tray;
    try {
        tray = new electron_1.Tray('web/favicon.png');
    }
    catch (e) {
        try {
            tray = new electron_1.Tray('resources/app/web/favicon.png');
        }
        catch (e) {
            try {
                tray = new electron_1.Tray(electron_1.app.getAppPath() + '/web/favicon.png');
            }
            catch (e) {
                logger_1.log('error', "Could not find favicon. appPath: " + electron_1.app.getAppPath());
...
const createSplashWindow = () => {
    ...
    const splashWindow = new electron_1.BrowserWindow({
        ...
        frame: false,
        show: false,
        backgroundColor: constants_1.BACKGROUND_COLOR,
        webPreferences: {
        ...
        icon: process.platform === 'linux' ? constants_1.LINUX_ICON_FILE : constants_1.ICON_FILEPATH,
    });
    /** once its ready to show, show */
    splashWindow.once('ready-to-show', () => {
        logger_1.log("debug", 'ready-to-show');
        splashWindow.show();
    });
    ...
    if (electron_1.app.isPackaged) {
        splashWindow.loadFile(constants_1.SPLASH_FILE);
    }
    else {
        /** development */
        //splashWindow.webContents.openDevTools();
        splashWindow.loadURL(`${constants_1.DEVELOPMENT_UI_URL}/splashscreen.html`);
    }
    /** Done */
    return splashWindow;
};
 ...
 *
 */
const createMainWindow = async (appPort) => {
    /** Create the browser window */
    const { width, height } = g_userSettings.get('windowBounds');
    const title = "Snapmail v" + electron_1.app.getVersion() + " - " + g_uid;
    const options = {
        height,
        width,
        title: constants_1.IS_DEBUG ? "[DEBUG] " + title : title,
        show: false,
        backgroundColor: constants_1.BACKGROUND_COLOR,
    ...
    let mainWindow = new electron_1.BrowserWindow(options);
    /** Things to setup at start */
    ...
    let mainUrl = electron_1.app.isPackaged ? constants_1.MAIN_FILE : path.join(constants_1.DEVELOPMENT_UI_URL, "index.html");
    mainUrl += "?APP=" + appPort + "&UID=" + g_uid;
    logger_1.log('info', "createMainWindow ; mainUrl = " + mainUrl);
    try {
        await mainWindow.loadURL("file://" + mainUrl);
    }
    catch (err) {
        logger_1.log('error', 'loadURL() failed:');
        logger_1.log('error', err);
    }
    /** Open <a href='' target='_blank'> with default system browser */
    mainWindow.webContents.on('new-window', function (event, url) {
        event.preventDefault();
        logger_1.log('info', "new-window ; open: " + url);
        electron_1.shell.openExternal(url).then(_r => { });
    });
    /** once its ready to show, show */
    ...
    mainWindow.on('close', async (event) => {
        ...
        if (g_canQuit) {
            logger_1.log('info', 'WINDOW EVENT "close" -> canQuit');
            //await tryToShutdown();
            mainWindow = null;
        }
        else {
    ...
    mainWindow.on('closed', async function () {
        logger_1.log('info', 'WINDOW EVENT "closed"');
        //await tryToShutdown();
 ...
 *
 */
async function startMainWindow(splashWindow) {
    /** Init conductor */
    const opts = holochain_1.createHolochainOptions(g_uid, g_sessionDataPath, g_networkSettings);
    logger_1.log('info', JSON.stringify(opts));
    const { statusEmitter, shutdown } = await electron_holochain_1.default(electron_1.app, opts, constants_1.BINARY_PATHS);
    g_statusEmitter = statusEmitter;
    g_shutdown = shutdown;
    statusEmitter.on(electron_holochain_1.STATUS_EVENT, async (state) => {
        //log('debug', "STATUS EVENT: " + stateSignalToText(state) + " (" + state + ")")
        switch (state) {
            case electron_holochain_1.StateSignal.IsReady:
                logger_1.log('debug', "STATUS EVENT: IS READY");
            ...
            default:
                if (splashWindow) {
                    splashWindow.webContents.send('status', holochain_1.stateSignalToText(state));
                }
        }
    });
    statusEmitter.on(electron_holochain_1.APP_PORT_EVENT, (appPort) => {
        //log('debug', "APP_PORT_EVENT: " + appPort)
        g_appPort = appPort;
    });
    statusEmitter.on(electron_holochain_1.ERROR_EVENT, (error) => {
        const error_msg = error;
        logger_1.log('error', error_msg);
        if (g_mainWindow == null && splashWindow) {
            splashWindow.webContents.send('status', error_msg);
        }
    });
    statusEmitter.on(electron_holochain_1.HOLOCHAIN_RUNNER_QUIT, () => {
    ...
    statusEmitter.on(electron_holochain_1.LAIR_KEYSTORE_QUIT, (e) => {
...
electron_1.app.on('ready', async () => {
    logger_1.log('debug', "ELECTRON READY - " + __dirname);
    /** Load user settings */
    g_userSettings = userSettings_1.loadUserSettings(1920, 1080);
    /** Show splashscreen */
    const splashWindow = createSplashWindow();
    /** init app */
    {
        const { sessionDataPath, uidList } = init_1.initApp(constants_1.USER_DATA_PATH, constants_1.APP_DATA_PATH, constants_1.DNA_VERSION_FILENAME, constants_1.RUNNING_ZOME_HASH_FILEPATH, constants_1.UID_LIST_FILENAME);
        g_sessionDataPath = sessionDataPath;
        g_uidList = uidList;
        g_dnaVersion = holochain_1.loadDnaVersion(sessionDataPath);
        /** Load network settings */
        const maybeNetworkSettings = networkSettings_1.loadNetworkConfig(g_sessionDataPath);
    ...
    logger_1.log('debug', "g_uid: " + g_uid);
    g_sessionDataPath = path.join(g_sessionDataPath, g_uid);
    logger_1.log('debug', "g_sessionDataPath: " + g_sessionDataPath);
    /** Create sys tray */
    g_tray = createTray();
    g_tray.setToolTip('Snapmail v' + electron_1.app.getVersion());
    const menu = electron_1.Menu.buildFromTemplate(trayMenuTemplate);
    ...
    logger_1.log('debug', "g_startingHandle found: " + g_startingHandle);
});
// /**
//  * This method will be called when Electron has finished initialization and is ready to create browser windows.
//  * Some APIs can only be used after this event occurs.
 ...
 */
electron_1.app.on('second-instance', (_event) => {
...
electron_1.app.on('will-quit', (event) => {
    logger_1.log('debug', '*** App "will-quit"');
    if (!g_canQuit) {
        event.preventDefault();
    }
});
...
electron_1.app.on('window-all-closed', async () => {
    logger_1.log('info', 'APP EVENT "window-all-closed"');
...
electron_1.app.on('activate', async () => {
    logger_1.log('debug', '*** App "activate"');
...
electron_1.app.on('before-quit', function () {
    logger_1.log('debug', '*** App "before-quit"');
    g_canQuit = true;
});
/**
 *
...
async function tryToShutdown() {
    try {
        logger_1.log('debug', { g_shutdown });
        if (g_shutdown) {
            logger_1.log('info', 'calling g_shutdown()...');
            await g_shutdown();
        }
    }
    catch (e) {
        logger_1.log('error', 'g_shutdown() failed: ' + e);
    }
}
async function restart() {
    logger_1.log('debug', "*** Restarting...");
    g_mainWindow = null;
    //g_statusEmitter = null;
    //await tryToShutdown();
    logger_1.log('debug', "*** Restarting: RELAUNCH");
    if (electron_1.app.isPackaged && process.env.APPIMAGE) {
        logger_1.log('debug', "*** ... with APPIMAGE: " + process.env.APPIMAGE);
...
async function promptNetworkType(canExitOnCancel) {
    const r = await electron_prompt_1.default({
        title: 'Select network type',
        height: 180,
        width: 300,
        alwaysOnTop: true,
        label: 'Choose network type:',
        icon: constants_1.CURRENT_DIR + constants_1.FAVICON_PATH,
        type: 'select',
        selectOptions: {
    ...
    if (r === null) {
        logger_1.log('debug', 'user cancelled. Can exit: ' + canExitOnCancel);
        if (canExitOnCancel) {
            electron_1.app.quit();
        }
    }
    else {
        logger_1.log('debug', 'promptNetworkType result: ' + r);
        g_networkSettings.canMdns = r === 'true';
        networkSettings_1.saveNetworkConfig(g_sessionDataPath, g_networkSettings);
    }
    return r !== null;
}
/**
...
async function promptBootstrapUrl(canExitOnCancel) {
    ...
    if (r === null) {
        ...
        logger_1.log('debug', 'result: ' + r);
        g_networkSettings.bootstrapUrl = r;
        networkSettings_1.saveNetworkConfig(g_sessionDataPath, g_networkSettings);
        const res = spawn_1.pingBootstrap(r);
        logger_1.log('info', 'bootstrap result: ' + res);
    }
    return r !== null;
}
/** */
async function promptFirstHandle() {
    ...
    if (r === null) {
        logger_1.log('debug', 'user cancelled. Exiting');
        electron_1.app.quit();
    }
    return r;
}
 ...
 * @returns false if user cancelled
 */
async function promptUid(canExitOnCancel, parentBrowserWindow) {
    const r = await electron_prompt_1.default({
    ...
    if (r === null) {
        ...
        const succeeded = init_1.addUidToDisk(r, sessionPath, constants_1.UID_LIST_FILENAME);
        if (succeeded) {
            g_uid = r;
            g_uidList.push(r);
            g_userSettings.set('lastUid', g_uid);
...
async function promptUidSelect(canExitOnCancel) {
    ...
    if (r === null) {
        ...
        logger_1.log('debug', 'promptUidSelect result: ' + r);
        g_uid = r;
...
async function promptCanProxy() {
    const { response } = await electron_1.dialog.showMessageBox(g_mainWindow, {
        title: `Proxy`,
    ...
    logger_1.log('warn', 'promptCanProxy: ' + response);
...
async function showAbout() {
    logger_1.log("info", `[${constants_1.RUNNER_VERSION}] DNA hash of "${g_uid}": ${g_dnaIdB64}\n`);
    await electron_1.dialog.showMessageBoxSync(g_mainWindow, {
        //width: 900,
        title: `About ${electron_1.app.getName()}`,
        message: `${electron_1.app.getName()} - v${electron_1.app.getVersion()}`,
        detail: `A minimalist email app on Holochain from Glass Bead Software\n\n`
            + `DNA Version:\n${g_dnaVersion}\n`
            + `DNA hash of "${g_uid}":\n${g_dnaIdB64}\n\n`
            + '' + constants_1.RUNNER_VERSION + ''
            + '' + constants_1.LAIR_VERSION + `\n`,
        buttons: ['OK'],
        type: "info",
        //iconIndex: 0,
        //icon: CONFIG.ICON,
...
async function confirmExit() {
    ...
    //let r = await prompt({
    const { response, checkboxChecked } = await electron_1.dialog.showMessageBox(g_mainWindow, {
        //width: 800,
        title: `Confirm Exit`,
 ...
 *
 */
async function promptHolochainError(browserWindow, msg) {
    await electron_1.dialog.showMessageBoxSync(browserWindow, {
        //width: 900,
        title: `Fatal Error`,
...
const debugMenuTemplate = [
    ...
    // },
    {
        ...
        click: function () {
            electron_1.shell.openExternal('file://' + constants_1.CONFIG_PATH);
    ...
    },
    {
        label: 'Open Log File',
        click: function () {
            electron_1.shell.openExternal('file://' + logger_1.electronLogger.transports.file.file);
    ...
    // },
    {
        ...
        click: async function () {
            ...
            const networkFilename = constants_1.NETWORK_URL.substring(constants_1.NETWORK_URL.lastIndexOf('/') + 1);
            //console.log({currentFilename})
            if (networkFilename != currentFilename) {
                await g_mainWindow.loadURL(constants_1.NETWORK_URL);
...
const mainMenuTemplate = [
    ...
    },
    {
        label: 'Help', submenu: [{
                ...
                click: function (menuItem, _browserWindow, _event) {
                    electron_1.shell.openExternal(constants_1.REPORT_BUG_URL);
...
/*  FINALIZE
/**********************************************************************************************************************/
electron_1.Menu.setApplicationMenu(electron_1.Menu.buildFromTemplate(mainMenuTemplate));
